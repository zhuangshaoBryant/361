...
import com.zhuang.safe361.activity.EnterPsdActivity;
	...
	private void watch() {
		...
		new Thread(){
			public void run() {
				...
				while(isWatch){
					/*
					getRunningTask方法在Android5.0以上已经被废弃，只会返回自己和系统的一些不敏感的task，
					不再返回其他应用的task，用此方法来判断自身App是否处于后台，仍然是有效的，但是无法判断其他应用是否位于前台，
					因为不再能获取信息
					 */
